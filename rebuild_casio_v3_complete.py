...
line_sales = df_casio.groupby('ライン')['販売数'].sum().sort_values(ascending=False)
if len(line_sales) > 7:
...
graphs_html = f'''
        ...
        <div class="casio-grid">
            ...
            <div class="casio-chart-container">
                <h4 class="casio-red">ライン別売上比率</h4>
                <div id="casio_line_chart" style="height: 350px;"></div>
            </div>
        </div>
'''
...
for line_name, data in sorted(casio_lines.items(), key=lambda x: x[1]['count'], reverse=True):
    ...
    lines_html += f'''
                    <tr>
                        ...
                        <td class="highlight casio-accent">¥{breakeven:,}</td>
                        <td>{cv:.3f}</td>
                        <td>{stability}</td>
                        <td>{jdm_ratio}</td>
    ...
    for i, model in enumerate(models, 1):
        ...
        line_models_html += f'''
                    <tr>
                        ...
                        <td class="highlight casio-accent">¥{model_breakeven:,}</td>
                        <td>{model['cv']:.3f}</td>
...
html = html[:start_pos] + new_casio_tab + html[end_pos:]
